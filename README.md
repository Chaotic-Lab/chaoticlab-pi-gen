...
2. Pre-installed `tdc-pulse-py` (from GitHub Releases of [Chaotic-Lab/chaoticlab-sbc](https://github.com/Chaotic-Lab/chaoticlab-sbc))
...
Both `chaoticlab-pi-gen` and `chaoticlab-sbc` are **private** (the IP is not exposed)
...
- `tdc-pulse` is **not** cloned from the private `chaoticlab-sbc` inside the image.
  CI checks out `chaoticlab-sbc` with a read-only token, builds the wheel, and
...
- `SBC_READ_TOKEN` — fine-grained PAT (or deploy token) with **read-only Contents**
  on `Chaotic-Lab/chaoticlab-sbc`, so CI can clone + build the wheel from the private lib
